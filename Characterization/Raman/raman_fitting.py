...
def fit(x,y,poly=0):
    ...
    if poly == 1:
        ax.plot(x, gaussian(x,popt[12],popt[13],popt[14]), alpha=0.5, label='TO 500: ' + str(int(popt[12])))
    ax.plot(x, lorentzian(x,520,popt[15],popt[16]), alpha=0.4, label='TO 520')
    ax.plot(x, gaussian(x,popt[17],popt[18],popt[19]), alpha=0.5, label='660: ' + str(int(popt[17])))
